...
calculateV k data_ = chiDist k v
  where
    ...
    V2 n sumY2 = Map.foldl' sumF (V2 0 0) data_ where
...
--
testUniformity
    :: forall a b. (Ord b, Show b, Typeable a)
    => QC.Gen a  -- ^ Generator to test
    -> (a -> b)  -- ^ Partitioning function
    -> Int       -- ^ Number of partittions
    -> TestTree
testUniformity gen f k = testProperty ("Uniform Arbitrary " ++ name)
    $ QC.forAllBlind (streamGen $ QC.scale (max 20) gen)
